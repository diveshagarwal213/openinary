...
| [`x`](#x)       | x offset      | Horizontal crop offset (pixels or %)        |
| [`y`](#y)       | y offset      | Vertical crop offset (pixels or %)          |
...
<a id="x"></a>
<ParamField path="x" type="integer | decimal | percentage">
  Horizontal offset for position-based cropping. Used with `c_crop` to define where the crop region starts.

| Format      | Example     | Meaning                        |
| ----------- | ----------- | ------------------------------ |
| Pixel       | `x_100`     | 100px from the left edge       |
| Decimal     | `x_0.3`     | 30% of the image width         |
| Percentage  | `x_30p`     | 30% of the image width         |

  <Note>
    `x` and `y` only take effect with `c_crop`. When specified, the `g` (gravity) parameter is ignored — manual coordinates override automatic positioning.
  </Note>
</ParamField>

<a id="y"></a>
<ParamField path="y" type="integer | decimal | percentage">
  Vertical offset for position-based cropping. Used with `c_crop` to define where the crop region starts.

| Format      | Example     | Meaning                        |
| ----------- | ----------- | ------------------------------ |
| Pixel       | `y_50`      | 50px from the top edge         |
| Decimal     | `y_0.2`     | 20% of the image height        |
| Percentage  | `y_20p`     | 20% of the image height        |
...
<Tab title="Position crop">

```http
GET /t/c_crop,w_300,h_300,x_100,y_50/image.jpg
```

Extracts a 300×300 region starting at pixel (100, 50) from the top-left.
